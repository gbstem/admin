...
**No email address is written to or read from Firestore any more**, apart from two deliberate exceptions: the contact details on applications and registrations (`personal.email`, `personal.secondaryEmail` — a student has no account of their own, and a second guardian needn't), and the backfill scripts under `scripts/`, which exist to clean up the addresses already stored. `isInstructorOfClass()` grants class writes on `instructorUid` alone, every notification resolves its recipient from a uid through Auth, and every view that shows somebody's address resolves it the same way — server-loaded pages through `resolveAccountEmails` in their DAL, browser-loaded ones through `/api/resolveEmails` (see [CLAUDE.md](CLAUDE.md)). The addresses still sitting in existing documents are dead weight, removed by running both backfill scripts with `--strip-emails`.
