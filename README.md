...
That same script also stamps a class's **primary** `instructorUid`, which was added after `instructorEmail` and so is absent on older documents. Unlike the co-instructor half, this must not change who can reach a class, so it stamps whoever currently owns `instructorEmail`; it falls back to the uid encoded in the class ID (the portal names classes `${uid}-${n}`, and the rules only allow creating one under your own uid) when that address no longer resolves to an account, which restores access that had already been silently lost.

**No email address is read from Firestore any more**, with one deliberate exception, and none is written except as an audit record. Applications and registrations still store `personal.email` - the applicant's, or for a registration the parent account's, address at the time of submission - but nothing reads it: it records what was submitted, nothing more. The exception is a registration's `personal.secondaryEmail`, a second guardian's address typed on the form. No Auth account backs it, so it can't be looked up and is frozen at submission, but it is the only way to reach that guardian, so it is read. (The account that registers a student is the parent's, though it holds the `student` role.) Everywhere else an address is needed it is resolved from a uid through Auth: `isOwnInterviewSlot`/`isInstructorOfClass()` decide ownership on uids alone, every notification resolves its recipient, and every view resolves the address it shows - server-loaded pages through `resolveAccountEmails`/`resolveRegistrationParentEmails` in their DAL, browser-loaded ones through `/api/resolveEmails` (see [CLAUDE.md](CLAUDE.md)). A registration's parent is found from its key, `${parentUid}-${n}` (`registrationParentUid`), and an application's applicant is its id. The backfill scripts under `scripts/` exist only to remove the addresses already stored on classes, slots, sub requests and slot requests, via `--strip-emails`.
...
**2. Never accept someone else's email address from the client.** Take their Auth `uid` instead and resolve the live address on the server with `adminAuth.getUser(uid)` — see [`accountEmail.ts`](src/lib/server/accountEmail.ts) and [`instructorDirectory.ts`](src/lib/server/instructorDirectory.ts) for the pattern. This fixes three separate problems at once: addresses stored in Firestore go stale the moment that person changes their account email, so reminders silently misdeliver; an address in a request body is chosen by whoever sent the request, so anyone could make gbSTEM's server send official-looking mail to any address on the internet; and a `uid` is meaningless to an attacker where an address is not.
...
| website    | `lib/semesterDates.json`                                     | `lib/courses.json`                         |
| curriculum | — (doesn't need it)                                          | `app/data/courses.json`                    |

Each downstream repo checks its own copies, and the website additionally checks that the copies still _match_ this repo: `gbstem/website`'s `.github/workflows/shared-data.yml` checks out this repo and diffs both files, on every PR there and nightly. No unit test can do that — a Jest run only sees its own checkout — and the nightly run is the point: the failure these files keep having is a rollover that updates admin and forgets a downstream repo, which produces no PR downstream for a PR-triggered check to run on. If you see that job red, the fix is always to re-run the `cp` for the file it names, never to edit either side by hand.

1. In **both the admin and portal repos**, update the `suffix` constant at the top of `src/lib/data/collections.ts` (e.g., `const suffix = 'Fall26'`). The website repo has no `suffix` — it has no Firestore access at all, so it never builds a collection path, and it reads the catalog only from a test rather than slicing it to a half.
2. In the **admin repo**:
   ...
   - Review and update the course catalog in [`courses.json`](src/lib/data/courses.json). You only need to touch the entries whose `semester` matches the half you're rolling into — adding a course means adding **both** its fall and spring entries, each with the `id` of its page in the curriculum repo. Then copy the file to the portal, curriculum and website repositories per the table above:
     ...
     cp src/lib/data/courses.json ../website/lib/courses.json
     ```
     ...
     A new course also fails the website's `__tests__/courses.test.tsx` until the public site advertises it. The website's URLs are editorial rather than derived from an `id` (one page usually covers both halves, math is a page per half, Lego Robotics is the `/robotics` track page itself), so that repo keeps a hand-written `COURSE_PAGES` table mapping each id to the URL that advertises it, and the test holds the table to the catalog in both directions and checks the page exists and is linked from its track page. Adding a course there is three things: a page under `app/<track>/`, a link from that track page's progression, and both ids in `COURSE_PAGES`. Nothing on the website reads the catalog at runtime — it is copied there purely so that test can exist.
